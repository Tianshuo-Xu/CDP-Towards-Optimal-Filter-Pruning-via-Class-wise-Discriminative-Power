...
class ModelProfiler:    
    ...
    def _get_result(self, m, flops, cin=None, cout=None):
        ...
        if cin != None and cout != None:
            result = {
            'flops': flops,
            'params': _get_params(m),
            'weight_shape': (cout, cin, m.weight.shape[2], m.weight.shape[3]),
        }
        else:
            result = {
                'flops': flops,
    ...
    def _count_convNd(self, m, x, y, name=None, prev_pruned=0):
        
        if torch.is_tensor(prev_pruned):
            prev_pruned = prev_pruned.item()
        #print(name, prev_pruned)
        cin = m.in_channels
        if prev_pruned <= m.in_channels:
            cin = m.in_channels - prev_pruned
        ...
        if hasattr(m, 'weight_mask'):
            cout = (m.weight_mask.view(m.weight_mask.shape[0], -1).sum(dim=1) != 0).sum(dim=0)
        total_ops = (cout * output_size * kernel_ops * cin) // m.groups # cout x oW x oH
                
        if torch.is_tensor(total_ops):
            total_ops = total_ops.item()
        
        if torch.is_tensor(cout):
            cout = cout.item()
        ...
        return self._get_result(m, total_ops, cin, cout)

    def _count_linear(self, m, x, y, prev_pruned=0):
    ...
    def count_module(self, m, x, y, name):
        ...
        if name.split('.')[1] in self.prev_nums:
            pass
        else:
            if self.prev_nums and x[0].size(1) == self.prev_channels[-1]:
                self.use_res = True
            self.prev_nums.append(name.split('.')[1])
            self.prev_channels.append(x[0].size(1))   
        
        # assume x is tuple of single tensor
        if hasattr(m, 'weight_mask') and type(m).__name__ == 'Conv2d':
                            
            result = self.ops[type(m)](m, x, y, name, self.prev_pruned)

            self.prev_pruned = (m.weight_mask.view(m.weight_mask.shape[0], -1).sum(dim=1) == 0).sum(dim=0)
            
            if 'conv.0' in name and self.use_res:
                self.prev_pruned = 0

        else:
            result = self.ops[type(m)](m, x, y, name)
        total_result = {
            'name': name,
...
def count_flops_params(model, x, custom_ops=None, verbose=True, mode='default'):
    ...
    if isinstance(x, tuple) and all(isinstance(t, int) for t in x):
        x = (torch.rand(x).to(original_device), )

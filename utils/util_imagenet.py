...
def train_epoch(net, lr, train_loader, test_loader):
    """
    Training a network
    ...
    optimizer = optim.SGD(net.parameters(), lr=lr, momentum=0.875, nesterov=False)
...
def train(net, epochs, lr, train_loader, test_loader, save_info='./', save_acc=68.0, start_epoch=0, device='cuda', log_every_n=50):
    """
    Training a network
    :param net: Network for training
    :param epochs: Number of epochs in total.
    ...
    optimizer = optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=4e-5, nesterov=True)
